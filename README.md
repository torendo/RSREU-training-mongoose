...
Так как в данной работе мы используем результат предыдущей, то нам необходимо выполнить [аналогичные действия](https://github.com/eugrdn/RSREU-training_Integration/#Шаг-zero----Подготовка-проекта) для установки зависимостей и старта проекта. Выполните последовательно две команды в консоли в папке проекта:
...
Старт проекта по адресу [http://localhost:8080](http://localhost:8080):
...
Готово - сервер базы данных ожидает подключения по адресу `http://localhost:27017`. Проверить подключение вы можете с помощью уже знакомого вам __Robo 3T__. А также в запущенном проекте по адресу из предыдущего шага должны появиться книги. 
