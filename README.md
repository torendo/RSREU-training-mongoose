...
### 1.3. Подключение модуля Mongoose
...
## 3. Модели и документы
 ...
 1. Для того чтобы перейти на использование схем в нашем приложении, нам необходимо модифицировать созданные в лабораторной работе №3 сервисы.
...
Также не забудьте импортировать используемые здесь модули: `mongoose` и `booksSchema`.

2. Теперь мы готовы работать с моделью коллекции книг, а также имеем на неё ссылку в нашем файле.

2.1. Перепишем метод _getAllBooks_ с использованием модели. С методами конструктора модели можно познакомиться в [документации](http://mongoosejs.com/docs/api.html#model_Model).
Итак, тело метода:
```javascript
...
4. Если в предыдущих пунктах всё сделано правильно, то к настоящему моменту мы должны получить рабочий вариант приложения, поведение будет таким же как и в лабораторной работе №3.
...
3. Добавьте проверку на длину для полей "author.firstName" и "author.lastName". Сделайте так, чтобы имя и фамилия автора считались невалидными, если они содержат цифры (используйте регулярное выражение [^0-9]).
...
8. Попытайтесь с помощью веб-интерфейса добавить книги, которые не смогут пройти валидацию (например, с пустым названием или фамилией автора с цифрами). Проверьте, была ли добавлена книга. Обратите внимание на вывоз консоли Node.js.
